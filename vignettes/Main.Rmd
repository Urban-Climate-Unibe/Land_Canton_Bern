...
Welcome to our reproducible workflow to compare different machine learning
techniques to a linear regression model. Furthermore, we want to explore if
machine learning is superior to multivariate linear regression. For this
purpose, the spatial temperature deviation of the city of Bern from the
MeteoSwiss measuring station in Zollikofen is modeled (urban heat island
effect). For the modeling temperature in the city of Bern, we use 5
meteorological variables from the measuring station in Zollikofen (2m
temperature, precipitation, global radiation, wind speed, and wind
direction) between the time period 2019--2022 and 10 different geospatial
data (6 land use classes and 4 geospatial layers). You can find an overview
in our proposal. The computing time is very high as the workflow processes
a large amount of data. We have therefore designed the workflow interface
to be interactive. The interface is built according to the following graph:

SCHEMATIC OF THE INTERFACE

Because we want also show a possible application of such models the best
model will be used to show spatial temperature distributions for a possible
day as a map.
...
again. It will install and load your package automatically.
...
We use all columns as a predictor except:
...
: **TABLE 1:** Overview about the reasons why a variable/column were
