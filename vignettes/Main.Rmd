...
processing. Figure 1 gives you a overview about the default mode and all
...
packages <- c("influxdbclient","ggplot2","tidyverse","lubridate","raster",
              ...
              "tidyterra","shiny", "xgboost", 'kableExtra','rnaturalearth')
```

### Change the Workflow EXPLAIN WORKFLOW? WHAT DO WE DO EXACTLY?
...
corresponding folder. If it does, then it will read in. If not, then three
questions will follow. Here, you can see those questions and an explanation
what the answers mean for the further workflow:

1.  As a default, you can knit this workflow in a demo version. That means
    we generated the land use classes and geospatial layer for you and
    stored them in a dropbox account. The demo mode will download the data
    from a dropbox account. The meteorologic data will be reduce to 5% of
    the origin data (random choice). We recommend it because the
    computational time can be very high. If you substitute "y" with "n",
    then you have several options to knit this workflow.

2.  If you changed knit from "y" to "n", then you can knit the workflow
    with all meteorologic data. The land use classes and the geospatial
    layers will be download the data from a dropbox account as well.

3.  

4.  You have a Combined.csv file already. Would you like to redo the
    proceeding?
...
5.  Do you want to generate all data by yourself? (It may takes up to 12
    hours)
...
6.  Do you want to proceed in a demo version?
...
interactive way when run, not knitted. First, tiff-files are generated.
...
downloading and processing of the tiff files as raw data from open sources.
...
ADD TABLE BURGER 2019?. Alternatively and preferably the tiffs are all
processed by the resolutions in !!!Table 1!!!. This results in three
predictors per variable in !!!table 1!!!.
...
|-------------------------|--------------------------------------------------|
| Log_Nr                            | does not make sens for spatial upscaling                                        |
...
# here go back to basics: what do we do exactly
...
Hyperparamater: all hyperparameters are tuned.
...
evaluation[[4]]
```
since random forest performes the best, we now look at the impact of variables
...
cor(combined_test$nn_prediction,combined_test$temperature)^2
RMSE(combined_test$nn_prediction,combined_test$temperature)
MAE(combined_test$nn_prediction,combined_test$temperature)
mean(combined_test$nn_prediction-combined_test$temperature)
}
```
...
|---------------|---------------|---------------|---------------|---------------|
| Linear Regression | 0.187          | 1.020           | 0.793          | 0.003           |
| KNN               | 0.179          | 1.047           | 0.790          | -0.006          |
| Random Forest     | 0.292          | 0.955           | 0.713          | -0.028          |
| XGB Boost         | \-             | \-              | \-             | \-              |
| Neuronal Network  | 0.099          | 1.143           | 0.890          | 0..0803         |
