...
@burger2021 with the same zonal mean per predictor class. The predictor class 
represents the original layer without processing, for example the vegetation height.
These are cut to the extent of the city and its surrounding lands as well as a spatial resolution of 5 meters and will be in the raw-data folder.
But since @burger2021 was selecting layers with nighttime mean modelling in mind this
...
Before starting this subchapter a brief clarification. This project uses cantonal land use data and federal geospatial data. These in their raw form are called predictor classes. These are then spatially averaged to obtain the actual predictors which are called geospatial layers or just predictors.
...
to the Git- Hub Repo. The original files can be found here:
<https://boris.unibe.ch/161882/> We use additional data from the AWS at
Zollikofen, as it is the official meteorologic station of Bern. The five
meteorological variables (2m temperature, precipitation, radiation and
winddirection and windspeed) with a temporal resolution of 10 minutes for
the years 2019-2022 will be used as well as the timestamp. Data of
temperature and precipitation of the previous six hours and twelve hours,
and one, three and five days and ten days (precipitation only) will also be
...
temperature distributions, zonally meaned geospatial data as shown in @burger2021 is used.
These are cantonal land use classes as well as federal geospatial data. All
of the data is spatially averaged/meaned to obtain the layers most effective as
shown in @burger2021 as well as our own take: generate three different geospatial
layers per predictor class. The federal and cantonal data is directly downloaded
from the web into R and processed. 

Table 1 provides a brief overview of the data we will
use. Zonal mean resolution is indicated with our owns first and then the distance used by @burger2021.


| Predictor class                                                      | Variable Type    | Zonal Mean Resolution / [3 predictors per class] [abbreviation] |
|------------------------------|-------------------|---------------------------|
...
=======
time period (see table 1). The network data is publicly available on
[BORIS](https://boris.unibe.ch/161882/), however since
...
and five days (for precipitation even ten days) will also be fed into the
...
goal is to predict the temperature anomaly of the on site location to the
...
regression, a knn and a random forest. Additionally, a XGBoost and a neural
network can be run.
...
geospatial layers as raw data from open sources. 
First the files are downloaded from either cantonal or federal swiss websites.
Then, the processing goes as follows:
1) The predictor classes layers are formatted to be separate files.
This means that for each land use class as defined by @burger2021 and for each 
geospatial layer from swisstopo there is a raster file which is the predictor class.
...
3) Each layer then is zonally meaned. This can be performed with the layer resolutions 
by @burger2021 which corresponds to one predictor per predictor class in table 1.
Alternatively the geospatial layers are all meaned by the resolutions in table 1 (3 mean layers per predictor class).
This then finally results in three (or one in case of @burger2021) geospatial layers per predictor class in table 1. 
The data of the predictor classes which corresponds to the raw layers can be 
found in the data-raw subfolder of this project for the land use classes.
The processed files can be found in the data folder in the subdirectory Tiffs.

#### Data combination

Data from MeteoSwiss for the Zollikofen meteostation is averaged to 1-hour
intervals for the variables: radiation, wind speed, wind direction, 2m
temperature, and precipitation (precipitation is summed). Subsequently,
rolling means/sums are calculated for 6, 12, 24 hours, 3 days, and 5 days
for both precipitation and temperature. Additionally, a 10-day rolling sum
is computed for precipitation to capture moisture comprehensively. This
processed data represents the MeteoSwiss meteorological dataset. Then data
from the LCD temperature loggers in the city are read in. This is then
combined with the **metadata**, resulting in a tidy data frame, where each
row corresponds to a measurement of a single logger to a single point in
time with its attributes such as its location as columns (from the
**metadata**). Then, the values corresponding to each location of the
loggers is read out of the geospatial predictors. This data is also added
to the data frame. This then is the finished Combined.csv with all data.

#### Variable Selection

First, variable selection was performed (see [variable
selection](Markdown::%20Variable_Selection.rmd)). This process was only
exculsively applied to the predictors listed i table 1, as layers by
@burger2021 were already reduced and selected. The variable selection using
the Boruta-algorithm revealed that, after removing variables for reasons
unrelated to relevance (refer to table 2), all remaining variables proved
to be relevant. While further variable selection was investigated, no
definitive conclusions were reached. Therefore, all generated predictors
are utilized. Table two provides a brief overview of why certain variables
were rejected. Based on this selection, a formula is now generated, which
is processed into a recipe. This is now the basis for creating the models.
...
through some questions. There are four main options when running the scrip:

-   **Demo mode (reduced data set, our geospatial layser) is the default
    verison**

-   **Regular mode (full data set, our geospatial layer) it is what we want
    to investiagte and all fine tuning, evaluations, and result are based
    on this mode)**

-   **Burger demo (5% of our meteorologic data but geospatial layers from
    @burger2021)**

-   **Burger regular mode: full data set ut geospatial layers from
    @burger2021**

**I know, you describe it later but I think we need an overview which is
crystal clear**
...
values [25m, 150m, 1000m] or with the zonal mean values as demonstrated by
@burger (see table1). This can be chosen in the console when run normally.
**Because the generation of the zonal means of the geospatial layers is
very time-intensive (20min - 2h, depends on your device), when knitting,
the files are always downloaded from an external source (Dropbox). When the
files are downloaded. The version with all geospatial layers is downloaded.
When run not knitted, the console will ask whether to download, or generate
the geospatial means, as well as which geospatial means.**
...
for the regular mode (all meteorologic data and our geospatial layers).
Consequently, if you choose to use the demo mode (and thus for knitting),
the models may be under- or overfitted and may not perform well. As
mentioned before, we distinguish between our basic models and our advanced
models.
...
target (local temperature). In the following subsections, we will introduce
you to these models. Please note that we utilize DoParallel due to the
extensive size of our data set. Additionally, the models are designed and
tuned to deliver optimal performance in regular mode. Consequently,
significant deviations are inevitable when operating in demo mode
exclusively.
=======
The basic models include multiple linear regression, k-nearest neighbors
(knn), and the random forest model. We will compare their performance based
on their ability to explain variance ($R^2$), accuracy (bias), and
precision (RMSE) in predicting the target (temperature anomaly). In the
following subsections, we will introduce you to these models. Please note
that we utilize DoParallel due to the extensive size of our data set.
...
data input to 150,000 rows and limits therefore the computational time to
...
variance ($R^2$), precision (RMSE), and accuracy (bias). Moreover, we
present additional the main statistic parameters. Once again, we
...
3.  The third element is a visualization of the model's performance on both
    ...
    good or worse. The orange line represents the ideal performance, and if
    all points lie on this line, the model predicts the target perfectly.
    The red line depicts the actual regression, and the closer it is to the
    orange line, the better the model.
...
Since random forest performed the best, we now look at the impact of
...
```
radiation behaves roughly as expected. A higher temperature in the logger/predicted data with higher radiation. This was known and has something to do with the bias of the logger. Interestingly the temperature prediction also increases when approaching zero radiation. A possible explanation is that because most loggers are placed in the city and the nighttime UHI-Effect is highest, the temperature then also increases in most loggers. Also for the windspeed this effect applies. Because most loggers are in the city, when the wind is lowest, the UHI is highest and therefore the predicted temperature increases. 
For the geospatial predictors, the garden 150 meters geospatial layers behaves as expected: the higher the value, the lower the predicted local temperature. For the seals surfaces (SE) 150 meters geospatial predictor makes the temperature increase with higher values. This is also expected.
...
: **TABLE 3:** Overview of the evaluation of the models in the
regular-mode.
...
geospatial layers), we observed that each model performs superiorly in
...
: **TABLE 4:** Overview of the evaluation of the models in the
Burger-regular-mode.

### Summary for the Demo Modes

To complete our analysis, we also summarize the results of the demo-mode
(table 5) and the burger-demo-mode (table 6).
...
: **TABLE 5:** Overview of the evaluation and results of the models with
...
: **TABLE 6:** Overview of the evaluation of the models with 5 % of the
data.
...
| Wind direction          | 200 ° (west wind) | $x[°] \in[0 \leq x \leq 360]$, with 0° = N, 180° = S |
