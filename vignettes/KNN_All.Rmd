...
combined <- combined |>
  mutate(hour = hour(time),
         month = month(time),
         day = day(time),
         temperature = temperature - tre200s0)

combined <- combined |>
    filter(complete.cases(across(all_of(tiff_names_short)))) |>
    dplyr::select(all_of(c(tiff_names_short,meteoswiss_names)),temperature,hour,month,day,Log_Nr) |> drop_na()
combined_test <- combined |> filter(time < ymd("20200101"))
combined_train <- combined |> filter(time > ymd("20200101"))
...
#   scale_y_continuous(expand = c(0, 0))
  ...
  ggplot(data = combined_test,
         aes(x = as.factor(Log_Nr), y = difference))+
    geom_boxplot()
...
meteo_pred <- c(0,10005,22.8,0.0,600,2.3,206,12,8,22)
