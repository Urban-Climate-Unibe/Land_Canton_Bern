...
CurrentMap <- function(){
    ...
    #reading in metadata
    meta <- read_csv("./data/Messnetz_2023_MF_230624.csv")|>
    ...
    points(combined$LV_03_E,combined$LV_03_N , pch = 16, cex = 0.7)
    text(combined$LV_03_E, combined$LV_03_N, round(combined$temperature,1), pos = 3,cex = 0.7, adj = c(1, 1))
    title(paste("R^2 = ",round(summary(model)$r.squared,2),", Mean Temp Logger = ",mean(round(combined$temperature),2),", Logger Count = ",length(combined$name)), currenttime)
    dev.off()
    #Night plot at 4:30 AM local time...
    if ((currenttime > Sys.Date()+lubridate::hm("02:30"))&&(currenttime < Sys.Date()+lubridate::hm("02:45"))) {
      tiff(filename = "D:/Foto2_NilsTinner/Logger_Network_Bern/analysis/NightMap.tif",width = 1000, height = 1000)
      extent <- rgdal::readOGR("D:/Foto2_NilsTinner/Logger_Network_Bern/data/Extent_Bern.shp")
      rivers <- rgdal::readOGR("D:/Foto2_NilsTinner/Logger_Network_Bern/data/Aare.shp")
      color = colorRampPalette(c("blue","deepskyblue", "white","orange", "red"))(100)
      raster::plot(out, col = color)
      sp::plot(extent, add = T)
      sp::plot(rivers, add = T)
      ...
      title(paste("R^2 = ",round(summary(model)$r.squared,2),", Mean Temp Logger = ",mean(round(combined$temperature),2),", Logger Count = ",length(combined$name)), currenttime)
      dev.off()
      drive_upload(media = "D:/Foto2_NilsTinner/Logger_Network_Bern/analysis/NightMap.tif", path = as_dribble("https://drive.google.com/drive/folders/1ZJqhB6zUb443ltvpOS-8tWQbX39M01bx"),overwrite = TRUE)
    }
