...
CurrentMap <- function(){
  ...
  tiffs <- list()
  for(coef in tiff_names_short){
    print(coef)
    tiffs[[coef]] <- raster::raster(paste("./data/Tiffs/",coef,".tif",sep = ""))
  }
  first_extent <- extent(tiffs[[7]])
  for (x in 1:length(tiffs)) {
    tiffs[[x]] <- crop(tiffs[[x]], first_extent)
  } #Somehow this does nothing, dont know hwy, or does it? maybe just continue...
  ...
  tiff(filename = "./analysis/CurrentMap_KNN.tif",width = 1000, height = 1000)
  ...
  title(paste("R^2 = ",round(mod_cv$results$Rsquared,2),", Mean Temp Logger = ",mean(round(combined$temperature),2),", Logger Count = ",length(combined$name),", K = ", mod_cv$bestTune), currenttime)
  dev.off()


  drive_upload(media = "./analysis/CurrentMap_KNN.tif", path = as_dribble("https://drive.google.com/drive/folders/1ZJqhB6zUb443ltvpOS-8tWQbX39M01bx"),overwrite = TRUE)
  ...
  if ((currenttime > with_tz(Sys.Date()+lubridate::hm("00:00")-lubridate::hm("02:00"),Sys.timezone()))&&(currenttime < with_tz(Sys.Date()+lubridate::hm("00:15")-lubridate::hm("02:00"),Sys.timezone()))) {
    tiff(filename = "D:/Foto2_NilsTinner/Logger_Network_Bern/analysis/MidNightMap_KNN.tif",width = 1000, height = 1000)
    extent <- rgdal::readOGR("./data/Extent_Bern.shp")
    rivers <- rgdal::readOGR("./data/Aare.shp")
    color = colorRampPalette(c("blue","deepskyblue", "white","orange", "red"))(100)
    raster::plot(temp, col = color)
    ...
    title(paste("R^2 = ",round(mod_cv$results$Rsquared,2),", Mean Temp Logger = ",mean(round(combined$temperature),2),", Logger Count = ",length(combined$name),", K = ", mod_cv$bestTune), currenttime)
    dev.off()
    drive_upload(media = "D:/Foto2_NilsTinner/Logger_Network_Bern/analysis/MidNightMap_KNN.tif", path = as_dribble("https://drive.google.com/drive/folders/1ZJqhB6zUb443ltvpOS-8tWQbX39M01bx"),overwrite = TRUE)
    lowest_temp <<- mean(combined$temperature)#reset every midnight
  }

  if(lowest_temp > mean(combined$temperature)) {
    tiff(filename = "D:/Foto2_NilsTinner/Logger_Network_Bern/analysis/ColdestMap_KNN.tif",width = 1000, height = 1000)
    extent <- rgdal::readOGR("./data/Extent_Bern.shp")
    rivers <- rgdal::readOGR("./data/Aare.shp")
    color = colorRampPalette(c("blue","deepskyblue", "white","orange", "red"))(100)
    raster::plot(temp, col = color)
    ...
    title(paste("R^2 = ",round(mod_cv$results$Rsquared,2),", Mean Temp Logger = ",mean(round(combined$temperature),2),", Logger Count = ",length(combined$name),", K = ", mod_cv$bestTune), currenttime)
    dev.off()
    drive_upload(media = "D:/Foto2_NilsTinner/Logger_Network_Bern/analysis/ColdestMap_KNN.tif", path = as_dribble("https://drive.google.com/drive/folders/1ZJqhB6zUb443ltvpOS-8tWQbX39M01bx"),overwrite = TRUE)
...
lowest_temp <- 50
packages <- c("influxdbclient","ggplot2","tidyverse","lubridate","raster","dplyr","googledrive","caret")
